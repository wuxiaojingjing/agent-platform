...
/** 保险服务域叶子（阶段 3b）。 */
public class InsuranceDomainAgent implements TechDomainAgent {
    private final InsuranceProductPort port;
    public InsuranceDomainAgent(InsuranceProductPort port) { this.port = port; }
    ...
    public TaskResult execute(UnifiedTask task) {
        ...
        try {
            ...
            return failed(task, "INSURANCE_BACKEND_UNAVAILABLE");
        }
    }
